...
function dangerCheckPackageJsonDependencyTargets(packageJsonFilePath) {
  ...
  for (const [depName, depTarget] of Object.entries(allDeps)) {
    ...
    if (depTargetContainsGit) {
      fail(`The package.json file ${packageJsonFilePath} contains a dependency or devDependency \`${depName}\` with a \`git+\` target`);
    }
    if (depTargetContainsHttp) {
      fail(`The package.json file ${packageJsonFilePath} contains a dependency or devDependency \`${depName}\` with a \`http://\` target`);
    }
    if (depTargetContainsHttps) {
      warn(`The package.json file ${packageJsonFilePath} contains a dependency or devDependency \`${depName}\` with a \`https://\` target`);
    }
  }
}
